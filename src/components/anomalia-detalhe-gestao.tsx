...
import { AlertTriangle, Loader2, History, CheckCircle2, Clock } from "lucide-react";
...
import type { Anomalia, AnomaliaAtualizacao, StatusAnomalia, Usuario } from "@/lib/checklist/types";
import { formatarDataHora } from "@/lib/checklist/format";
import { updateAnomaliaTratativa } from "@/lib/checklist/supabase-storage";
import { useAnomaliaAtualizacoes } from "@/hooks/use-anomalia-atualizacoes";

const STATUS_EDITAVEIS: StatusAnomalia[] = ["Em andamento", "Resolvida"];

type FormState = {
  status: StatusAnomalia;
  responsavel: string;
  oQueFoiFeito: string;
  resolvidoEm: string; // datetime-local
};
...
export function AnomaliaDetalheGestao({
  anomalia,
  usuario,
  onUpdated,
}: {
  ...
  const initial = useMemo<FormState>(
    () => ({
      status: STATUS_EDITAVEIS.includes(anomalia.status) ? anomalia.status : "Em andamento",
      responsavel: anomalia.responsavelManutencao ?? "",
      oQueFoiFeito: anomalia.oQueFoiFeito ?? "",
      resolvidoEm: anomalia.resolvidoEm
        ? toLocalInput(anomalia.resolvidoEm)
        : nowLocalInput(),
    }),
    [anomalia],
  );
  ...
  const { data: historico, loading: loadingHistorico, refetch: refetchHistorico } = useAnomaliaAtualizacoes(anomalia.id);
  ...
  function validar(): boolean {
    ...
    if (form.status === "Resolvida") {
      const iso = localInputToISO(form.resolvidoEm);
      if (!iso) novosErros.resolvidoEm = "Informe data e hora válidas";
      else if (new Date(iso).getTime() < new Date(anomalia.criadoEm).getTime())
        novosErros.resolvidoEm = "Não pode ser anterior à abertura";
    }
    setErros(novosErros);
  ...
  async function salvar() {
    ...
    setSalvando(true);
    try {
      await updateAnomaliaTratativa(anomalia.id, {
        status: form.status,
        responsavelManutencao: form.responsavel.trim(),
        oQueFoiFeito: form.oQueFoiFeito.trim(),
        resolvidoEm:
          form.status === "Resolvida" ? localInputToISO(form.resolvidoEm) ?? undefined : undefined,
        atualizadoPorLogin: usuario.usuario,
        atualizadoPorPerfil: "gestao",
        origemHorarioResolucao: form.status === "Resolvida" ? "manual_gestao" : undefined,
      });
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border border-border bg-card p-5 shadow-sm md:p-6">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <Info
            ...
            valor={anomalia.ultimaAtualizacaoEm ? formatarDataHora(anomalia.ultimaAtualizacaoEm) : "—"}
          />
          <Info
      ...
      <div className="rounded-2xl border border-border bg-card p-5 shadow-sm md:p-6">
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Select
              value={form.status}
              onValueChange={(v) => setForm((s) => ({ ...s, status: v as StatusAnomalia }))}
            >
          ...
          {form.status === "Resolvida" && (
            <div className="space-y-2">
              <Label htmlFor="resolvido">Concluída em *</Label>
              <Input
                id="resolvido"
                type="datetime-local"
                value={form.resolvidoEm}
                onChange={(e) => setForm((s) => ({ ...s, resolvidoEm: e.target.value }))}
                className="w-full md:w-72"
              />
              {erros.resolvidoEm && (
                <p className="text-sm text-destructive">{erros.resolvidoEm}</p>
              )}
              <p className="text-xs text-muted-foreground">
                Informe manualmente o horário em que a manutenção foi concluída.
              </p>
            </div>
          )}
      ...
      {/* Histórico */}
      <div ref={historicoRef} className="scroll-mt-24 rounded-2xl border border-border bg-card p-5 shadow-sm md:p-6">
        <h2 className="mb-4 flex items-center gap-2 text-lg font-bold text-foreground">

...
- Testes locais em 25/09: a suíte completa passou com 144 testes, `npx tsc --noEmit` e `npm run build` passaram. Uma consulta REST anônima a `producao_horaria` retornou zero linhas, mas isso não prova o acesso correto dos operadores. `supabase/sql/verificar_prontidao_hora_x_hora.sql` foi preparado para conferir operadores e horas confirmadas sem retornar nomes ou producao. Ainda faltam teste no Supabase real, tablet e Telegram.

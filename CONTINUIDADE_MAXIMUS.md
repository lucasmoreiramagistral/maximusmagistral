...
- O usuário aplicou `20260922101000_empacotadora_verso.sql` em 24/09. A verificação posterior retornou: ambas as tabelas presentes, 6 policies, 4 gatilhos, RLS nas 2 tabelas e 0 registros iniciais em cada uma. Isso confirma a estrutura, **não** o funcionamento das policies com logins reais nem que o formulário salva e relê corretamente. A versão aplicada foi preparada para aceitar rascunhos parciais e exigir fechamento consistente. Os 5 perfis de gestão são reconhecidos por `is_gestao`; líder, supervisor e manutenção não recebem acesso de leitura novo ao verso. Testar com operador de cada empacotadora, operador de outra máquina e gestão antes da publicação.
- A migration `20260923100000_telegram_hora_publicacoes.sql` e as Edge Functions `hora-x-hora-telegram` e `hora-x-hora-publico` estão preparadas no código, mas **não aplicadas, implantadas nem agendadas**. O token do bot exposto em captura antiga deve ser revogado no BotFather. `scripts/obter-chat-id-telegram.ps1` pede o token novo sem ecoá-lo e mostra somente o ID do grupo; não enviar o token ao chat ou ao Git. O `project_id` em `supabase/config.toml` foi alinhado ao projeto mostrado no SQL Editor; na implantação, conferir explicitamente o projeto de destino.
- O usuário decidiu que qualquer pessoa com o link do card pode abrir o Hora x Hora e que o link não expira. A página `/hora-x-hora-publico` é somente de leitura, limitada a um dia operacional, com operador, produto, cadência, quantidade, perda equivalente e motivo; observações livres, assinaturas e formulários internos ficam fora. O link contém token aleatório guardado em tabela sem acesso de `anon`/`authenticated`, pode ser repassado e pode ser desativado com `revogado_em`. A consulta usa Edge Function com acesso de serviço somente no servidor. A página e a função ainda não foram testadas no Supabase real nem publicadas no Lovable.
- Em 25/09, o salvamento Hora x Hora foi separado em `INSERT` para hora nova e `UPDATE` para linha existente. A assinatura posterior envia somente campos do líder; a atualização de linha existente usa `updated_at` como guarda de concorrência, e uma resposta perdida é confirmada por releitura. Isso evita passar um registro já finalizado pelo ramo de `INSERT` do gatilho. Testes locais desse caminho passaram, mas ainda falta exercitar a gravação real no Supabase.
- Em 25/09, a seleção da folha foi ajustada para não tentar assumir uma linha em branco criada por outro operador do mesmo equipamento. Uma hora já preenchida continua visível para o turno seguinte; uma hora em branco de outro login usa um novo ID de lançamento. O contrato de data/faixa entre app e Telegram foi testado nas 24 horas do dia operacional, incluindo a virada de ano.
- O usuário executou `supabase/sql/verificar_prontidao_hora_x_hora.sql` no banco real em 25/09. Resultado: Enchedora 2, Empacotadora 2 e Empacotadora 3 têm **0 operadores ativos, 0 horas confirmadas e 0 horas legadas**; Enchedora 3 tem **16 operadores ativos, 0 horas confirmadas e 20 horas legadas preenchidas**. Não há teste de escrita do fluxo novo. Não alterar nem apagar esses 20 registros para ensaiar. Criar contas de teste dedicadas para as três máquinas novas antes de validar acesso, gravação e Telegram.
- Testes locais em 25/09: a suíte completa passou com 148 testes, `npx tsc --noEmit` e `npm run build` passaram. Uma consulta REST anônima a `producao_horaria` retornou zero linhas, mas isso não prova o acesso correto dos operadores. Ainda faltam teste com contas reais no Supabase, tablet e Telegram.

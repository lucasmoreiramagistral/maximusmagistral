...
- A pré-checagem de `20260922101000_empacotadora_verso.sql` passou em 24/09: as duas tabelas ainda não existem, e `maximus_operador_da_maquina`, `is_gestao` e `touch_updated_at` existem. Os 5 perfis de gestão são reconhecidos por `is_gestao`; líder, supervisor e manutenção não recebem acesso de leitura novo ao verso. **Ainda não foi aplicada a migration 3.** A versão atual do arquivo permite salvar rascunhos parciais de consolidação, mas exige fechamento completo e total correto. Depois de aplicar, executar `supabase/sql/verificar_empacotadora_verso_apos.sql` e testar com logins reais de operador e gestão.
